...
//      R[y * width + x] = R[y * width + x] * (1.0 / (max - min)) - min / (max - min);
      R[y * width + x] = (R[y * width + x] - min) / (max - min);
                         ...
                         int* features)
{
    ...
    cudaMemcpy(output, cudaOutputAux, width * height * sizeof(int), cudaMemcpyDeviceToHost);
...
__global__ void fletar_bordes(float * const aux,
                              const size_t width,
                              const size_t heigth)
{
    const unsigned int x = blockIdx.x * blockDim.x + threadIdx.x;
...
void nonMaximaSupression()
{
  ...
  fletar_bordes<<<gridSize, blockSize>>>(cudaOutputAux,
                                         width,
                                         height);
  ...
  std::cout<<"Despues de normalizar:"<<std::endl;
  img = thrust::device_pointer_cast(cudaOutputAux);
  max_elem = thrust::max_element(img, img + width * height);
  min_elem = thrust::min_element(img, img + width * height); 

  const float max1 = *max_elem;
  const float min1 = *min_elem;
